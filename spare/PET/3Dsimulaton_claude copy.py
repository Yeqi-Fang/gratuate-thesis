...
class PETSimulator:
    ...
    def simulate_events(self, num_events: int) -> np.ndarray:
        ...
        Returns:
            Array of shape (N, 11) containing:
        ...
        for _ in range(num_events):
            ...
            if det1 is not None and det2 is not None:
                ...
                det1_pos = self.detector_positions[det1]
                det2_pos = self.detector_positions[det2]
                
                # Store all information
                event_data = [
                    det1, det2,                    # Detector IDs
                    det1_pos[0], det1_pos[1], det1_pos[2],  # Detector 1 position
                    det2_pos[0], det2_pos[1], det2_pos[2],  # Detector 2 position
                    x_pos, y_pos, z_pos            # Event position
                ]
...
def save_events_with_positions(filename: str, events: np.ndarray):
    ...
    # Detector IDs as integers, positions as floats
    fmt = ['%d', '%d'] + ['%.6f']*9
...
def main():
    ...
    # Simulate events
    events = simulator.simulate_events(num_events=20)
    
    # Save events with positions
